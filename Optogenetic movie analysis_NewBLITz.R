...
plotDir <- "/Users/jonathantownson/Documents/PhD/CSL localisation Paper/Figures for paper/Files/Fig 4"
setwd(mainDir)
...
df <- filter(AllSummarisedData,LaserActivation %in% c("458nm20","476nm20","488nm20","496nm20","NoActivation"))
df$'Time (minutes)' <- df$Frame*(TimeMin/(length(df$Frame)/length(unique(df$LaserActivation))))
df$LaserActivation <- gsub("458nm20", "458 nm", df$LaserActivation)
df$LaserActivation <- gsub("476nm20", "476 nm", df$LaserActivation)
df$LaserActivation <- gsub("488nm20", "488 nm", df$LaserActivation)
df$LaserActivation <- gsub("496nm20", "496 nm", df$LaserActivation)
...
ggplot(df, aes(`Time (minutes)`,`Mean NuclearAccumulation`,group = Wavelength, col=Wavelength,shape=Wavelength,
  ...
  labs(title = "OptIC-Notch{ω} activation", x = "Time (mins)", y = "Nuclear Accumulation") +
  scale_color_brewer(palette = "Set2")
