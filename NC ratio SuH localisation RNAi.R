...
mainDir <- "/Users/jonathantownson/Documents/PhD/CSL localisation Paper/Figures for paper/Files/Fig 1"
setwd(mainDir)
# Create the path where the data is stored
DataPath <- paste(mainDir, "RNAi quantification", sep = "/")
# Set the X axis order
XAxisOrder <- c("LacZ wRNAi", "LacZ HRNAi", "NDECD wRNAi","NDECD HRNAi",
                "OldBLITz wRNAi", "OldBLITz HRNAi", "NewBLITz wRNAi","NewBLITz HRNAi")
...
LacZ_wRNAi <- SuHdata %>% dplyr::filter(IncubationTime == "LacZ wRNAi")
LacZ_HRNAi <- SuHdata %>% dplyr::filter(IncubationTime == "LacZ HRNAi")
NDECD_wRNAi <- SuHdata %>% dplyr::filter(IncubationTime == "NDECD wRNAi")
NDECD_HRNAi <- SuHdata %>% dplyr::filter(IncubationTime == "NDECD HRNAi")
OldBLITz_wRNAi <- SuHdata %>% dplyr::filter(IncubationTime == "OldBLITz wRNAi")
OldBLITz_HRNAi <- SuHdata %>% dplyr::filter(IncubationTime == "OldBLITz HRNAi")
NewBLITz_wRNAi <- SuHdata %>% dplyr::filter(IncubationTime == "NewBLITz wRNAi")
NewBLITz_HRNAi <- SuHdata %>% dplyr::filter(IncubationTime == "NewBLITz HRNAi")
...
NGCytratio_crossbar <- function(Data, Labels) {
  ggplot(Data,aes(IncubationTime,NGCytratio,col=IncubationTime,shape=IncubationTime)) + theme_jmt() +
    geom_jitter(width=0.25) +
    stat_summary(fun="mean", geom="crossbar", width=0.5) + theme(legend.key=element_blank())  + 
    labs(title = Labels[[1]], x = Labels[[2]], y = Labels[[3]]) + 
    scale_color_brewer(palette = "Set2")
...
NuclearPercentIntDen_crossbar <- function(Data, Labels) {
  ggplot(Data,aes(IncubationTime,NuclearPercentIntDen,col=IncubationTime,shape=IncubationTime)) + theme_jmt() +
    geom_jitter(width=0.25) +
    stat_summary(fun="mean", geom="crossbar", width=0.5) + theme(legend.key=element_blank())  + 
    labs(title = Labels[[1]], x = Labels[[2]], y = Labels[[3]]) + 
    scale_color_brewer(palette = "Set2")
}
NuclearPercentGlandIntDen_crossbar <- function(Data, Labels) {
  ggplot(Data,aes(IncubationTime,NuclearPercentGlandIntDen,col=IncubationTime,shape=IncubationTime)) + theme_jmt() +
    geom_jitter(width=0.25) +
    stat_summary(fun="mean", geom="crossbar", width=0.5) + theme(legend.key=element_blank())  + 
    labs(title = Labels[[1]], x = Labels[[2]], y = Labels[[3]]) + 
    scale_color_brewer(palette = "Set2")
}
NuclearPercentGlandCytIntDen_crossbar <- function(Data, Labels) {
  ggplot(Data,aes(IncubationTime,NuclearPercentGlandCytIntDen,col=IncubationTime,shape=IncubationTime)) + theme_jmt() +
    geom_jitter(width=0.25) +
    stat_summary(fun="mean", geom="crossbar", width=0.5) + theme(legend.key=element_blank())  + 
    labs(title = Labels[[1]], x = Labels[[2]], y = Labels[[3]]) + 
    scale_color_brewer(palette = "Set2")
...
NucAreaPerc_crossbar <- function(Data, Labels) {
  ggplot(Data,aes(IncubationTime,NucAreaPerc,col=IncubationTime,shape=IncubationTime)) + theme_jmt() +
    geom_jitter(width=0.25) +
    stat_summary(fun="mean", geom="crossbar", width=0.5) + theme(legend.key=element_blank())  + 
    labs(title = Labels[[1]], x = Labels[[2]], y = Labels[[3]]) + 
    scale_color_brewer(palette = "Set2")
}
NCratioComp <- function(Data, Labels) {
  ggplot(Data, aes(Gland, NCratio, col=IncubationTime,shape=IncubationTime)) + theme_jmt() + 
    geom_point()  +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
    labs(title = Labels[[1]], x = Labels[[2]], y = Labels[[3]])
}
NGratioComp <- function(Data, Labels) {
  ggplot(Data, aes(Gland, NGratio, col=IncubationTime,shape=IncubationTime)) + theme_jmt() + 
    geom_point()  +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
    labs(title = Labels[[1]], x = Labels[[2]], y = Labels[[3]])
}
GlandMeanComp <- function(Data, Labels) {
  ggplot(Data, aes(`Gland`, `Gland Mean`, col=IncubationTime,shape=IncubationTime)) + theme_jmt() + 
    geom_point()  +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
    labs(title = Labels[[1]], x = Labels[[2]], y = Labels[[3]])
}
...
#### Saved plots ####
setwd("/Users/jonathantownson/Documents/PhD/CSL localisation Paper/Figures for paper/Files/Fig 1")

df <- rbind(LacZ_wRNAi,LacZ_HRNAi,NDECD_wRNAi,NDECD_HRNAi)
df1list <- list()
for (g in 1:length(unique(df$IncubationTime))) {
  df1list[[g]] <- df[str_detect(df$IncubationTime, as.character(unique(df$IncubationTime)[g])),]
  names(df1list)[g] <- as.character(unique(df$IncubationTime)[g])
}
levels(df$IncubationTime) <- gsub("NDECD", "Nact", levels(df$IncubationTime))
Labs <- c(title = "eGFP::CSL", x = "Incubation Time", y = "Nuclear:Cytoplasmic ratio")
NCratio_crossbar(df, Labs) + theme(axis.text.x = element_text(angle = 0, vjust = 0.5, hjust=0.5), axis.title.x=element_blank(), legend.position = "none") + 
  guides(colour = guide_legend(nrow=2, byrow = TRUE)) +
  geom_signif(comparisons = list(c("LacZ wRNAi","LacZ HRNAi")),y_position = 4.75, 
              map_signif_level=TRUE, test = "t.test", color = "black") +
  geom_signif(comparisons = list(c("Nact wRNAi","Nact HRNAi")),y_position = 4.75, 
              map_signif_level=TRUE, test = "t.test", color = "black") +
  geom_signif(comparisons = list(c("LacZ wRNAi","Nact wRNAi")),y_position = 5.15, 
              map_signif_level=TRUE, test = "t.test", color = "black") +
  geom_signif(comparisons = list(c("LacZ HRNAi","Nact HRNAi")),y_position = 5.55, 
              map_signif_level=TRUE, test = "t.test", color = "black") + 
  scale_y_continuous(breaks = seq(0, 6, by = 2), limits = c(0,6))
ggsave(paste("wRNAi and HRNAi with LacZ or NDECD NCratio of",CoFactor,".jpg"), device = "jpeg", dpi = "retina",
       width = 15, height = 15, units = "cm")

NormDis_plots(df1list, "NCratio", "IncubationTime",paste("Normal distribution of",CoFactor," NCratio plots for LacZ vs Nact in wRNAi or HRNAi"))
QQ_plots(df1list, "NCratio", "IncubationTime",paste("QQ plots of",CoFactor," NCratio plots for LacZ vs Nact in wRNAi or HRNAi"))
DataStatSumTestListFun(df1list,paste("Summary of",CoFactor," for LacZ vs Nact in wRNAi or HRNAi"))
Stats_tests(df1list,paste("SigStats of",CoFactor," for LacZ vs Nact in wRNAi or HRNAi"))
rm(df, df1list)

df <- rbind(OldBLITz_wRNAi,OldBLITz_HRNAi,NewBLITz_wRNAi,NewBLITz_HRNAi)
df1list <- list()
for (g in 1:length(unique(df$IncubationTime))) {
  df1list[[g]] <- df[str_detect(df$IncubationTime, as.character(unique(df$IncubationTime)[g])),]
  names(df1list)[g] <- as.character(unique(df$IncubationTime)[g])
}
levels(df$IncubationTime) <- gsub("OldBLITz", "OptIC-Notch \n", levels(df$IncubationTime))
levels(df$IncubationTime) <- gsub("NewBLITz", "OptIC-Notch{ω} \n", levels(df$IncubationTime))
Labs <- c(title = "eGFP::CSL", x = "Incubation Time", y = "Nuclear:Cytoplasmic ratio")
NCratio_crossbar(df, Labs) + theme(axis.text.x = element_text(angle = 0, vjust = 0.5, hjust=0.5), axis.title.x=element_blank(), legend.position = "none") + 
  guides(colour = guide_legend(nrow=2, byrow = TRUE)) +
  geom_signif(comparisons = list(c("OptIC-Notch \n wRNAi","OptIC-Notch \n HRNAi")),y_position = 4.75, 
              map_signif_level=TRUE, test = "t.test", color = "black") +
  geom_signif(comparisons = list(c("OptIC-Notch{ω} \n wRNAi","OptIC-Notch{ω} \n HRNAi")),y_position = 4.75, 
              map_signif_level=TRUE, test = "t.test", color = "black") +
  geom_signif(comparisons = list(c("OptIC-Notch \n wRNAi","OptIC-Notch{ω} \n wRNAi")),y_position = 5.15, 
              map_signif_level=TRUE, test = "t.test", color = "black") +
  geom_signif(comparisons = list(c("OptIC-Notch \n HRNAi","OptIC-Notch{ω} \n HRNAi")),y_position = 5.55, 
              map_signif_level=TRUE, test = "t.test", color = "black") + 
  scale_y_continuous(breaks = seq(0, 6, by = 2), limits = c(0,6))
ggsave(paste("wRNAi and HRNAi with OldBLITz or NewBLITz NCratio of",CoFactor,".jpg"), device = "jpeg", dpi = "retina",
       width = 15, height = 15, units = "cm")

NormDis_plots(df1list, "NCratio", "IncubationTime",paste("Normal distribution of",CoFactor," NCratio plots for OldBLITz vs NewBLITz in wRNAi or HRNAi"))
QQ_plots(df1list, "NCratio", "IncubationTime",paste("QQ plots of",CoFactor," NCratio plots for OldBLITz vs NewBLITz in wRNAi or HRNAi"))
DataStatSumTestListFun(df1list,paste("Summary of",CoFactor," for OldBLITz vs NewBLITz in wRNAi or HRNAi"))
Stats_tests(df1list,paste("SigStats of",CoFactor," for OldBLITz vs NewBLITz in wRNAi or HRNAi"))
rm(df, df1list)
