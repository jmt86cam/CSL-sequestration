...
DataPath <- paste(mainDir, "New WT vs NBM quantification", sep = "/")
# Set the X axis order
XAxisOrder <- c("WT control", "WT BLITz", "NBM control","NBM BLITz")
...
NBM_control <- SuHdata %>% dplyr::filter(IncubationTime == "NBM control")
NBM_BLITz <- SuHdata %>% dplyr::filter(IncubationTime == "NBM BLITz")
WT_control <- SuHdata %>% dplyr::filter(IncubationTime == "WT control")
WT_BLITz <- SuHdata %>% dplyr::filter(IncubationTime == "WT BLITz")
...
levels(df$IncubationTime) <- gsub(" BLITz", "", levels(df$IncubationTime))
...
NCratio_crossbar(df, Labs) + theme(axis.text.x = element_text(angle = 0, vjust = 0.5, hjust=0.5), axis.title.x=element_blank(), legend.position = "none") + 
  ...
  geom_signif(comparisons = list(c("WT","NBM")),y_position = 5.5, 
              map_signif_level=TRUE, test = "t.test", color = "black") + 
  scale_y_continuous(breaks = seq(0, 6, by = 2), limits = c(0,6))
ggsave(paste("NBM and WT with OptIC Notch NCratio of",CoFactor,".jpg"), device = "jpeg", dpi = "retina",
       width = 15, height = 15, units = "cm")

NormDis_plots(df1list, "NCratio", "IncubationTime",paste("Normal distribution of",CoFactor," NCratio plots for NBM vs WT"))
QQ_plots(df1list, "NCratio", "IncubationTime",paste("QQ plots of",CoFactor," NCratio plots for NBM vs WT"))
DataStatSumTestListFun(df1list,paste("Summary of",CoFactor," for NBM vs WT"))
Stats_tests(df1list,paste("SigStats of",CoFactor," for NBM vs WT"))
rm(df, df1list)
